...
} from "../repositories/oauthToken.repository.js";
...
export async function saveUserTokens(email: string, tokens: any): Promise<{ id: number }> {
  return upsertTokens(email, tokens);
}
...
export async function getAuthForUser(email: string): Promise<OAuth2Client | { success: false; message: string }> {
  ...
  // Load user tokens
  const tokenRow = await getTokenByEmail(email);
  ...
  oAuth2Client.setCredentials({
    ...
    // Credentials.scope is `string | undefined` (not nullable) — omit it when null
    // rather than passing null (also respects exactOptionalPropertyTypes).
    ...(tokenRow.scope != null ? { scope: tokenRow.scope } : {}),
